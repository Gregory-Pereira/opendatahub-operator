...
const Type = cluster.SelfManagedRhoai

var _ platform.Platform = (*SelfManaged)(nil)
...
// Init performs platform-specific initialization for self-managed deployments.
func (p *SelfManaged) Init(ctx context.Context) error {
	// Initialize services
...
// Run executes platform-specific runtime logic for self-managed deployments.
func (p *SelfManaged) Run(ctx context.Context, mgr ctrl.Manager) error {
...
// setupResources creates default resources (DSCI and Gateway) for self-managed deployments.
// Errors are logged but don't block startup (non-blocking).
func (p *SelfManaged) setupResources(ctx context.Context) error {
...
// Validator returns the platform-specific validator for self-managed deployments.
func (p *SelfManaged) Validator() platform.Validator {
	return &validator{}
}

// Type returns the platform type identifier for self-managed deployments.
func (p *SelfManaged) Type() common.Platform {
	return Type
}

// String returns the canonical platform display name for self-managed deployments.
func (p *SelfManaged) String() string {
	return string(Type)
}
