...
```bash
oc get va autoscaling-example-qwen-kserve-va
...
It should be noted for your `VariantAutoscaling` it should show `METRICSREADY=True` and your scaledObject should be showing as `READY=true`. Once the WVA controller starts reconciling the `scaledObject` it should also become `ACTIVE=True`. If you are not seeing that, your first debugging steps should be to `describe` these resources and check their `Status` sections of their yaml.
...
### Token-based Saturation Analyzer
[TODO]