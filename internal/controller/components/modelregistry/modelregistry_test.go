...
func TestNewCRObject(t *testing.T) {
	...
	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.ModelRegistryInstanceName),
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			dsc := createDSCWithModelRegistry(tt.state)
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle enabled component with ready ModelRegistry CR", func(t *testing.T) {
		...
		dsc := createDSCWithModelRegistry(operatorv1.Managed)
		modelregistry := createModelRegistryCR(true)

		cli, err := fakeclient.New(fakeclient.WithObjects(dsc, modelregistry))
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.modelregistry.managementState == "%s"`, operatorv1.Managed),
	...
	t.Run("should handle enabled component with not ready ModelRegistry CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithModelRegistry(operatorv1.Managed)
		modelregistry := createModelRegistryCR(false)
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithModelRegistry(operatorv1.Removed)
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.modelregistry.managementState == "%s"`, operatorv1.Removed),
			jq.Match(`.status.conditions[] | select(.type == "%s") | .status == "%s"`, ReadyConditionType, metav1.ConditionFalse),
	...
	t.Run("should handle empty management state as Removed", func(t *testing.T) {
		...
		dsc := createDSCWithModelRegistry("")
...
func createDSCWithModelRegistry(managementState operatorv1.ManagementState) *dscv2.DataScienceCluster {
	dsc := dscv2.DataScienceCluster{}
	dsc.SetGroupVersionKind(gvk.DataScienceCluster)
	dsc.SetName("test-dsc")

	dsc.Spec.Components.ModelRegistry.ManagementState = managementState

	return &dsc
}

func createModelRegistryCR(ready bool) *componentApi.ModelRegistry {
