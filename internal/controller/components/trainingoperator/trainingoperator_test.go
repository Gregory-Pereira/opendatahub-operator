...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.TrainingOperatorComponentName))
}
...
func TestNewCRObject(t *testing.T) {
	...
	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.TrainingOperatorInstanceName),
		jq.Match(`.kind == "%s"`, componentApi.TrainingOperatorKind),
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			dsc := createDSCWithTrainingOperator(tt.state)
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithTrainingOperator(operatorv1.Removed)
	...
	t.Run("should handle empty management state as Removed", func(t *testing.T) {
		...
		dsc := createDSCWithTrainingOperator("")
...
func createTrainingOperatorCR(ready bool) *componentApi.TrainingOperator {
	c := componentApi.TrainingOperator{}
	c.SetGroupVersionKind(gvk.TrainingOperator)
	c.SetName(componentApi.TrainingOperatorInstanceName)
