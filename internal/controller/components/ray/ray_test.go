...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.RayComponentName))
}
...
func TestNewCRObject(t *testing.T) {
	...
	g.Expect(cr).Should(BeAssignableToTypeOf(&componentApi.Ray{}))

	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.RayInstanceName),
		jq.Match(`.kind == "%s"`, componentApi.RayKind),
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle enabled component with ready Ray CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()
	...
	t.Run("should handle enabled component with not ready Ray CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithRay(operatorv1.Managed)
		ray := createRayCR(false)
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithRay(operatorv1.Removed)
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.ray.managementState == "%s"`, operatorv1.Removed),
			jq.Match(`.status.conditions[] | select(.type == "%s") | .status == "%s"`, ReadyConditionType, metav1.ConditionFalse),
...
func createDSCWithRay(managementState operatorv1.ManagementState) *dscv2.DataScienceCluster {
	...
	dsc.Spec.Components.Ray.ManagementState = managementState

	return &dsc
}

func createRayCR(ready bool) *componentApi.Ray {
	c := componentApi.Ray{}
	c.SetGroupVersionKind(gvk.Ray)
	c.SetName(componentApi.RayInstanceName)
