...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.DataSciencePipelinesComponentName))
}
...
func TestNewCRObject(t *testing.T) {
	...
	g.Expect(cr).Should(BeAssignableToTypeOf(&componentApi.DataSciencePipelines{}))

	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.DataSciencePipelinesInstanceName),
		jq.Match(`.kind == "%s"`, componentApi.DataSciencePipelinesKind),
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			dsc := createDSCWithDataSciencePipelines(tt.state)
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle enabled component with ready DataSciencePipelines CR", func(t *testing.T) {
		...
		cli, err := fakeclient.New(fakeclient.WithObjects(dsc, datasciencepipelines))
...
func createDSCWithDataSciencePipelines(managementState operatorv1.ManagementState) *dscv2.DataScienceCluster {
...
func createDataSciencePipelinesCR(ready bool) *componentApi.DataSciencePipelines {
	c := componentApi.DataSciencePipelines{}
	c.SetGroupVersionKind(gvk.DataSciencePipelines)
	c.SetName(componentApi.DataSciencePipelinesInstanceName)
